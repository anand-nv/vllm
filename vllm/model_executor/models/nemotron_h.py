...
class NemotronHModel(nn.Module):
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, input_asr_ids: torch.Tensor | None = None) -> torch.Tensor:
        emb = self.embed_tokens(input_ids)
        if input_asr_ids is not None:
    ...
    def forward(
        ...
        if get_pp_group().is_first_rank:
            if inputs_embeds is not None:
                ...
                hidden_states = self.embed_input_ids(input_ids, input_asr_ids)
            residual = None
        else:
...
class NemotronHForCausalLM(
    ...
    def embed_input_ids(self, input_ids: torch.Tensor, input_asr_ids: torch.Tensor | None = None) -> torch.Tensor:
        return self.model.embed_input_ids(input_ids, input_asr_ids)
