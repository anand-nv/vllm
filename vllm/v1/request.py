...
class Request:
    ...
    def read_custom_inputs(
        ...
        The custom_inputs_ready flag is only cleared once all tokens have
        been read.

        Args:
            num_scheduled_tokens: Number of tokens being scheduled in this
                iteration. If None, returns all custom inputs.
        ...
        for input_name, input_tensor in self.custom_inputs.items():
            if end_idx > input_tensor.shape[0]:
                raise ValueError(
                    f"Custom input {input_name} has only "
                    f"{input_tensor.shape[0]} tokens, tried to read "
                    f"[{start_idx}:{end_idx}]"
                )
